...
import stringifyValidationErrors from './stringify-validation-errors';
...
export class ErrorDetails {
  ...
  addDebugInfo(debugInfo: DebugInfo) {
    this.debugInfos.push(debugInfo);
    return this;
  }
...
export function toErrorDetailsHttpException(
  ...
  if (exception instanceof ErrorDetails) return exception.toHttpException();
  ...
  if (Array.isArray(exception) && exception[0] instanceof ValidationError) {
    return ErrorDetails.from(
      StatusCode.INVALID_ARGUMENT,
      stringifyValidationErrors(exception),
    ).toHttpException();
  }
